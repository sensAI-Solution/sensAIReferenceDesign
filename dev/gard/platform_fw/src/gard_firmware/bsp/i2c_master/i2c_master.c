...
uint8_t i2c_master_init(struct i2cm_instance *this_i2cm, uint32_t base_addr)
{
	if (NULL == this_i2cm) {
		return 1;
	}
						  ...
						  uint16_t              pre_scaler)
{
	...
	if (this_i2cm->addr_mode != i2c_mode) {
		...
		if (i2c_mode == I2CM_ADDR_10BIT_MODE) {
			reg_8b_modify(this_i2cm->base_address | REG_MODE, I2C_ADDR_MODE,
						  I2C_ADDR_MODE);
		}
	}

	// set the pre_scaler to tune the i2c clock
	reg_8b_modify(this_i2cm->base_address | REG_MODE, I2C_CLK_DIV_HIGH,
				  pre_scaler >> 8);
						...
						uint8_t              *data_buffer)
{
	...
	reg_8b_write(this_i2cm->base_address | REG_SLAVE_ADDR_LOW, address & 0x7F);
	...
	while (1) {
		...
		if ((fifo_status & RX_FIFO_EMPTY_MASK) == 0) {
			if (data_count <= read_length) {
				reg_8b_read(this_i2cm->base_address | REG_DATA_BUFFER,
							data_buffer);
			}
...
uint8_t i2c_master_repeated_start(struct i2cm_instance *this_i2cm,
								  ...
								  uint8_t              *wr_data_buffer,
								  uint8_t               rd_data_size,
								  uint8_t              *rd_data_buffer)
{
	...
	while (data_count < wr_data_size) {
		...
		// if tx fifo is full, stop loading fifo for now, resume in interrupt or
		// polling loop
		...
		reg_8b_write(this_i2cm->base_address | REG_DATA_BUFFER,
					 *wr_data_buffer);  // push the data into tx buffer
	...
	while (1) {
		// cycle completes when all bytes are transmitted or a NACK is received
		...
		if (data_count < wr_data_size) {
			// load any additional bytes into tx fifo when it becomes almost
			// empty
			reg_8b_read(this_i2cm->base_address | FIFO_STATUS_REG,
						&fifo_status);
			if (fifo_status &
				TX_FIFO_AEMPTY_MASK)  // if tx fifo is almost empty
			{
				reg_8b_write(this_i2cm->base_address | REG_DATA_BUFFER,
							 *wr_data_buffer);  // push the data into tx buffer
						 ...
						 uint8_t              *data_buffer)
{
	...
	while (data_count < data_size) {
		...
		reg_8b_write(this_i2cm->base_address | REG_DATA_BUFFER,
					 *data_buffer);  // push the data into tx buffer
...
void i2c_master_isr(void *ctx)
{
	...
	if (i2c_int2 & I2C_ERR) {
		// reset the i2c master
		reg_8b_modify(this_i2cm->base_address | REG_CONFIG, I2C_MASTER_RESET,
					  I2C_MASTER_RESET);
