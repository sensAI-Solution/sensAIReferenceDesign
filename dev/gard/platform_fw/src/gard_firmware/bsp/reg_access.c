...
uint8_t reg_32b_read(uint32_t reg_addr, uint32_t *reg_32b_value)
{
	uint32_t volatile *reg_value = (void *)reg_addr;
...
uint8_t reg_32b_modify(uint32_t reg_addr, uint32_t bits_mask, uint32_t value)
{
	uint32_t volatile *reg_value = (void *)reg_addr;

	//*reg_value = ((*reg_value) & ~bits_mask) | value;
	/* <TBD_KDB> Temporary fix to set GPIO pin 1 HIGH
	 * However limitations shall be fixed on higher index pins */
	*reg_value = ((*reg_value) & (~bits_mask)) | (value << (bits_mask - 1));
	return 0;
}
...
uint8_t reg_16b_read(uint32_t reg_addr, unsigned short *reg_16b_value)
{
	unsigned short volatile *reg_value = (void *)reg_addr;
...
uint8_t reg_8b_modify(uint32_t reg_addr, uint8_t bits_mask, uint8_t value)
{
	uint8_t volatile *reg_value = (void *)reg_addr;
...
uint8_t reg_8b_read(uint32_t reg_addr, uint8_t *reg_8b_value)
{
	uint8_t volatile *reg_value = (void *)reg_addr;
