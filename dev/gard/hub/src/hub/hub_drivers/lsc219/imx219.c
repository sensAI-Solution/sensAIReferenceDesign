 ...
 * Copyright (C) 2018 Intel Corporation
 *
 * LSC219 modifications: All I2C callouts divereted to locally defined stub
 * functions that return success (0)
 * I2C Unbinding: I2C Client reference is changed to use platform device.
...
/* Define printk wrapper for lattice specific imx219 driver */
#define lsc_pr(fmt, args...)                                                   \
	pr_info("LSC219: %s:%d:%s(): " fmt, __FILE__, __LINE__, __func__, ##args)
/* Define for I2C Unbinding, Set to 0 */
#define LSC219_I2C_BINDING						  0x00
...
/* IMX219 Metadata Pad format */
#define IMX219_NUM_EMBEDDED_LINE_WIDTH			   16384
...
struct imx219 {
	...
	struct media_pad           pad[IMX219_NUM_PADS];
...
static int imx219_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct platform_device          *client = v4l2_get_subdevdata(&imx219->sd);
...
static int imx219_init_controls(struct imx219 *imx219)
{
	...
	imx219->pixel_rate = v4l2_ctrl_new_std(
		...
		imx219_get_pixel_rate(imx219), imx219_get_pixel_rate(imx219), 1,
	...
	imx219->vblank = v4l2_ctrl_new_std(
		ctrl_hdlr, &imx219_ctrl_ops, V4L2_CID_VBLANK, IMX219_VBLANK_MIN,
		IMX219_VTS_MAX - mode->height, 1, mode->vts_def - mode->height);
	hblank = IMX219_PPL_DEFAULT - mode->width;
	imx219->hblank =
		v4l2_ctrl_new_std(ctrl_hdlr, &imx219_ctrl_ops, V4L2_CID_HBLANK, hblank,
						  hblank, 1, hblank);
	if (imx219->hblank) {
	...
	exposure_def     = (exposure_max < IMX219_EXPOSURE_DEFAULT)
						   ? exposure_max
						   : IMX219_EXPOSURE_DEFAULT;
	imx219->exposure = v4l2_ctrl_new_std(
		ctrl_hdlr, &imx219_ctrl_ops, V4L2_CID_EXPOSURE, IMX219_EXPOSURE_MIN,
		exposure_max, IMX219_EXPOSURE_STEP, exposure_def);
	...
	for (i = 0; i < 4; i++) {
		...
		v4l2_ctrl_new_std(ctrl_hdlr, &imx219_ctrl_ops,
						  ...
						  IMX219_TESTP_COLOUR_MIN, IMX219_TESTP_COLOUR_MAX,
						  IMX219_TESTP_COLOUR_STEP, IMX219_TESTP_COLOUR_MAX);
...
{
	fmt->format.width  = IMX219_NUM_EMBEDDED_LINE_WIDTH;
								  ...
								  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	else if(fse->pad == IMX219_PAD_META)
	{
		if (fse->code != MEDIA_BUS_FMT_SENSOR_DATA || fse->index > 0)
			return -EINVAL;

		fse->min_width 	= IMX219_NUM_EMBEDDED_LINE_WIDTH;
		fse->max_width  = fse->min_width;
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad >= IMX219_NUM_PADS) {
		dev_err(&client->dev, "%s invalid num pad: %d\n", __func__,
				fmt->pad);
		return -EINVAL;
	}

	if(fmt->pad == IMX219_PAD_IMAGE) {
			return v4l2_subdev_get_fmt(sd, sd_state, fmt);
		} else if(fmt->pad == IMX219_PAD_META){
			imx219_update_metadata_pad_format(fmt);
		}
	return 0;
}
...
{
	struct platform_device *client = v4l2_get_subdevdata(&imx219->sd);
	u64                val = IMX219_CHIP_ID;

	if (val != IMX219_CHIP_ID) {
		return dev_err_probe(&client->dev, -EIO, "chip id mismatch: %x!=%llx\n",
							 IMX219_CHIP_ID, val);
	}
...
static int imx219_check_hwcfg(struct device *dev, struct imx219 *imx219)
{
	...
	case 2:
		ret = v4l2_link_freq_to_bitmap(
			dev, ep_cfg.link_frequencies, ep_cfg.nr_of_link_frequencies,
			imx219_link_freq_menu, ARRAY_SIZE(imx219_link_freq_menu),
			&link_freq_bitmap);
		break;
	case 4:
		ret = v4l2_link_freq_to_bitmap(
			dev, ep_cfg.link_frequencies, ep_cfg.nr_of_link_frequencies,
			imx219_link_freq_4lane_menu,
			ARRAY_SIZE(imx219_link_freq_4lane_menu), &link_freq_bitmap);

		if (!ret && (link_freq_bitmap & BIT(1))) {
			dev_warn(dev,
					 "Link frequency of %d not supported, but has been "
					 "incorrectly advertised previously\n",
					 IMX219_DEFAULT_LINK_FREQ_4LANE_UNSUPPORTED);
...
static int imx219_probe(struct platform_device *client)
{
	struct device *dev = &client->dev;
	struct imx219 *imx219;
	int            ret;

	lsc_pr("Inside %s", __func__);
	...
	v4l2_subdev_init(&imx219->sd, &imx219_subdev_ops);
	imx219->sd.owner = dev->driver->owner;
	imx219->sd.dev = dev;
	imx219->sd.internal_ops = &imx219_internal_ops;
	v4l2_set_subdevdata(&imx219->sd, client);

	dev_set_drvdata(dev, &imx219->sd);

	/* Initialize name */
         snprintf(imx219->sd.name, sizeof(imx219->sd.name), "%s",
	          dev->driver->name);
	...
	imx219->pad[IMX219_PAD_IMAGE].flags          = MEDIA_PAD_FL_SOURCE;

	/* Initialize Metadata pad */
	imx219->pad[IMX219_PAD_META].flags =	MEDIA_PAD_FL_SOURCE;

	ret = media_entity_pads_init(&imx219->sd.entity, IMX219_NUM_PADS, imx219->pad);
	if (ret) {
...
static void imx219_remove(struct platform_device *client)
{
	//struct v4l2_subdev *sd     = i2c_get_clientdata(client);
	struct imx219 		 *imx219 = platform_get_drvdata(client);
	//struct imx219      *imx219 = to_imx219(sd);
	struct v4l2_subdev *sd       = &imx219->sd;
...
module_platform_driver(imx219_platform_driver);

MODULE_AUTHOR("Dave Stevenson <dave.stevenson@raspberrypi.com");
MODULE_AUTHOR("Prasad BM <prasad.munirathnam@latticesemi.com>");
MODULE_AUTHOR("Deepak Nayak <deepak.nayak@latticesemi.com>");
MODULE_DESCRIPTION("Lattice IMX219 sensor driver");
