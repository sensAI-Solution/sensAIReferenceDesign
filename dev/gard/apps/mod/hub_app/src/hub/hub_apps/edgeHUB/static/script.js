...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const updateDashboard = async () => {
        try {
            ...
            const metadataCheckboxBox = document.getElementById('metadata-checkbox-box');
            if(metadataCheckboxBox) {
                metadataCheckboxBox.classList.toggle('hidden', !data["isFpgaEnabled"]);
            }
            const metadataCheckbox = document.getElementById('metadata-checkbox');
            if(metadataCheckbox) {
                metadataCheckbox.checked = data["isDisplayingAIMetadata"];
            }
    ...
    function enableMetadata(enabled) {
        fetch('/enable_metadata', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        ...
        .then(response => response.json())
        .catch(error => {
            console.error('Error enabling metadata:', error);
        });
    }
    ...
    if( metadataCheckbox ){
        metadataCheckbox.addEventListener('change', (e) => {
            enableMetadata(e.target.checked);
        });
    }
