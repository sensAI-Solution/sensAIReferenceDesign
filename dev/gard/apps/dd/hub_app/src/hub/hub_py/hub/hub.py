...
from .eveInterface import EVEInterface
...
class HUB:
    ...
    eve_dir_default_paths = glob.glob("/opt/EVE-*-Source/bin", recursive=False)
    ...
    def __init__(
        self, host_config_file: str = None, config_file_path: str = None, eve_bin_path: str = None
    ) -> None:
        ...
        eve_bin_path = eve_bin_path or (
            self.eve_dir_default_paths[0] if self.eve_dir_default_paths else "")
        ...
        # Create an EVEInterface instance
        if eve_bin_path and os.path.isdir(eve_bin_path):
            self.eve_instance = EVEInterface(eve_bin_path, toJpg=False, copyImage=False, maxWidth=1280)
            self.eve_instance.init(useMetadataCamera=False)
            
            # Events Monitoring
            context = "EVE"
            BUFFER_SIZE = 256
            self.events_buffer = bytearray(BUFFER_SIZE)
            
            gard_num = self.get_gard_count()
            if not gard_num:
                self.logger.info("No GARDs discovered, cannot continue!")
            else:                
                gard_num = 0
                gard0 = self.get_gard(gard_num)
                if gard0:
                    self.setup_appdata_callback(gard0, self.__appdata_callback_for_eve, self.eve_instance, self.events_buffer)
                    self.logger.info("Events monitoring has started.")
        else:
            self.eve_instance = None
    ...
    @staticmethod
    def __appdata_callback_for_eve(ctx, buffer):
        eve_instance = ctx
        if eve_instance:
            eve_instance.send_manual_buffer(buffer)
