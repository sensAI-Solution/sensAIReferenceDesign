...
void *hub_gpio_monitor_thread_func(void *hub_mon_params)
{
	...
	int loops = 0;
	while (loops++ < 10 && !p_hub_gpio_mon_ctx->terminate_flag) {	
		if (loops > 1)
		{
			hub_pr_dbg("Loops: %d\n", loops);
			// wait 100ms if there's a possible wait time, slowed down by a mutex or something.
			usleep(100000);
		}
